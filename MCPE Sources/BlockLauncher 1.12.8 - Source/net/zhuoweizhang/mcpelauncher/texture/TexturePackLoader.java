...
public class TexturePackLoader
{
  ...
  public static List<TexturePack> loadTexturePacks(Context paramContext, List<String> paramList, byte[] paramArrayOfByte1, byte[] paramArrayOfByte2)
    throws Exception
  {
    paramArrayOfByte1 = metaToList(paramArrayOfByte1);
    paramArrayOfByte2 = metaToList(paramArrayOfByte2);
    Object localObject = loadDescriptions(paramContext);
    ArrayList localArrayList = new ArrayList(((List)localObject).size());
    localObject = ((List)localObject).iterator();
    while (((Iterator)localObject).hasNext())
    {
      TexturePackDescription localTexturePackDescription = (TexturePackDescription)((Iterator)localObject).next();
      TexturePack localTexturePack = loadTexturePack(localTexturePackDescription);
      if (!isCompatible(localTexturePack, paramArrayOfByte1, paramArrayOfByte2)) {
        paramList.add(describeTexturePack(paramContext, localTexturePackDescription));
      } else {
        localArrayList.add(localTexturePack);
      }
    }
    return localArrayList;
  }
