...
public class ActivityLifecycleTracker
{
    ...
    paramActivity = new Runnable()
    {
      public void run()
      {
        if (ActivityLifecycleTracker.currentSession == null)
        {
          ...
          if (localObject != null) {
            SessionLogger.logDeactivateApp(this.val$activity, (SessionInfo)localObject, ActivityLifecycleTracker.appId);
          }
          ActivityLifecycleTracker.access$302(new SessionInfo(Long.valueOf(this.val$currentTime), null));
          localObject = SourceApplicationInfo.Factory.create(this.val$activity);
          ActivityLifecycleTracker.currentSession.setSourceApplicationInfo((SourceApplicationInfo)localObject);
          SessionLogger.logActivateApp(this.val$activity, (SourceApplicationInfo)localObject, ActivityLifecycleTracker.appId);
        }
      }
    };
        ...
        if (ActivityLifecycleTracker.foregroundActivityCount.get() <= 0)
        {
          Runnable local1 = new Runnable()
          {
            public void run()
            {
              if (ActivityLifecycleTracker.foregroundActivityCount.get() <= 0)
              {
                SessionLogger.logDeactivateApp(ActivityLifecycleTracker.4.this.val$activity, ActivityLifecycleTracker.currentSession, ActivityLifecycleTracker.appId);
                SessionInfo.clearSavedSessionFromDisk();
                ActivityLifecycleTracker.access$302(null);
              }
            }
          };
          ActivityLifecycleTracker.access$702(ActivityLifecycleTracker.singleThreadExecutor.schedule(local1, ActivityLifecycleTracker.access$500(), TimeUnit.SECONDS));
        }
  ...
  private static void onActivityResumed(Activity paramActivity)
  {
    ...
    paramActivity = new Runnable()
    {
      public void run()
      {
        this.val$activity.getCallingActivity();
        if (ActivityLifecycleTracker.currentSession == null)
        {
          ActivityLifecycleTracker.access$302(new SessionInfo(Long.valueOf(this.val$currentTime), null));
          SessionLogger.logActivateApp(this.val$activity, null, ActivityLifecycleTracker.appId);
        }
        for (;;)
        {
          ActivityLifecycleTracker.currentSession.setSessionLastEventTime(Long.valueOf(this.val$currentTime));
          ...
          if (ActivityLifecycleTracker.currentSession.getSessionLastEventTime() != null)
          {
            ...
            if (l > ActivityLifecycleTracker.access$500() * 1000)
            {
              SessionLogger.logDeactivateApp(this.val$activity, ActivityLifecycleTracker.currentSession, ActivityLifecycleTracker.appId);
              SessionLogger.logActivateApp(this.val$activity, null, ActivityLifecycleTracker.appId);
              ActivityLifecycleTracker.access$302(new SessionInfo(Long.valueOf(this.val$currentTime), null));
            }
            else if (l > 1000L)
...
/* Location:              C:\Users\Genbu Hase\ÉhÉLÉÖÉÅÉìÉg\Genbu\Tool\Programing\Jad\Minecraft PE 0.15.3.2.jar!\com\facebook\appevents\internal\ActivityLifecycleTracker.class
