...
public class CorrelationVector
{
  ...
  {
    int i = (int)Math.floor(Math.log10(this.currentVector) + 1.0D);
    return this.baseVector.length() + 1 + i + 1 + 1 <= SettingsStore.getCllSettingsAsInt(SettingsStore.Settings.MAXCORRELATIONVECTORLENGTH);
  }
  ...
  private boolean CanIncrement(int paramInt)
  {
    ...
    {
      return false;
      paramInt = (int)Math.floor(Math.log10(paramInt) + 1.0D);
    } while (this.baseVector.length() + paramInt + 1 > SettingsStore.getCllSettingsAsInt(SettingsStore.Settings.MAXCORRELATIONVECTORLENGTH));
    return true;
  }
...
/* Location:              C:\Users\Genbu Hase\ÉhÉLÉÖÉÅÉìÉg\Genbu\Tool\Programing\Jad\Minecraft PE 0.15.3.2.jar!\com\microsoft\cll\android\CorrelationVector.class
