...
public class DeviceIdentityManager
{
    ...
    throws NetworkException, InvalidResponseException, StsException
  {
    ...
    DeviceProvisionRequest localDeviceProvisionRequest = null;
    int i = 1;
    for (;;)
    {
      ...
      if (i <= 3)
      {
        localObject1 = getCredentialGenerator().generate();
        if (localDeviceProvisionRequest != null) {
          break label87;
        }
        localDeviceProvisionRequest = getRequestFactory().createDeviceProvisionRequest((DeviceCredentials)localObject1);
      }
      for (;;)
      {
        DeviceProvisionResponse localDeviceProvisionResponse = (DeviceProvisionResponse)localDeviceProvisionRequest.send();
        if (!checkProvisionResponse(i, localDeviceProvisionResponse)) {
          break;
        }
        localObject1 = new DeviceIdentity((DeviceCredentials)localObject1, localDeviceProvisionResponse.getPuid(), null);
        this._storage.writeDeviceIdentity((DeviceIdentity)localObject1);
        return (DeviceIdentity)localObject1;
        label87:
        localDeviceProvisionRequest.setDeviceCredentials((DeviceCredentials)localObject1);
      }
      i += 1;
    }
  }
...
/* Location:              C:\Users\Genbu Hase\ƒhƒLƒ…ƒƒ“ƒg\Genbu\Tool\Programing\Jad\Minecraft PE 0.15.3.2.jar!\com\microsoft\onlineid\sts\DeviceIdentityManager.class
