  ...
  implements TexturePack
{
  ...
  public ModPkgTexturePack(String paramString)
  {
    ...
    throws IOException
  {
    ...
    if (paramString.startsWith(this.prefix)) {
      str = paramString.substring(this.prefix.length());
    }
    if (filterTheseOut.contains(str)) {
      return null;
    }
    paramString = this.subPacks.iterator();
    while (paramString.hasNext())
    {
      InputStream localInputStream = ((ZipTexturePack)paramString.next()).getInputStream(str);
