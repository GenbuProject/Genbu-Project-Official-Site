...
public final class ResponseHeaders
{
  ...
  private long computeAge(long paramLong)
  {
    ...
    if (this.ageSeconds != -1) {
      l = Math.max(l, java.util.concurrent.TimeUnit.SECONDS.toMillis(this.ageSeconds));
    }
    for (;;)
    {
      return l + (this.receivedResponseMillis - this.sentRequestMillis) + (paramLong - this.receivedResponseMillis);
    }
  }
  ...
  private long computeFreshnessLifetime()
  {
    ...
    if (this.maxAgeSeconds != -1) {
      l1 = java.util.concurrent.TimeUnit.SECONDS.toMillis(this.maxAgeSeconds);
    }
    label72:
    do
    {
  ...
  public ResponseSource chooseResponseSource(long paramLong, RequestHeaders paramRequestHeaders)
  {
    ...
    if (paramRequestHeaders.getMaxAgeSeconds() != -1) {
      paramLong = Math.min(l1, java.util.concurrent.TimeUnit.SECONDS.toMillis(paramRequestHeaders.getMaxAgeSeconds()));
    }
    l1 = 0L;
    if (paramRequestHeaders.getMinFreshSeconds() != -1) {
      l1 = java.util.concurrent.TimeUnit.SECONDS.toMillis(paramRequestHeaders.getMinFreshSeconds());
    }
    long l3 = 0L;
    long l2 = l3;
    if (!this.mustRevalidate)
    {
      ...
      if (paramRequestHeaders.getMaxStaleSeconds() != -1) {
        l2 = java.util.concurrent.TimeUnit.SECONDS.toMillis(paramRequestHeaders.getMaxStaleSeconds());
      }
    }
    if ((!this.noCache) && (l4 + l1 < paramLong + l2))
    {
      if (l4 + l1 >= paramLong) {
        this.headers.add("Warning", "110 HttpURLConnection \"Response is stale\"");
      }
      if ((l4 > com.integralblue.httpresponsecache.compat.java.util.concurrent.TimeUnit.HOURS.toMillis(24L)) && (isFreshnessLifetimeHeuristic())) {
  ...
  public ResponseHeaders combine(ResponseHeaders paramResponseHeaders)
  {
    ...
    i = 0;
    while (i < paramResponseHeaders.headers.length())
    {
      str1 = paramResponseHeaders.headers.getFieldName(i);
      if (isEndToEnd(str1)) {
